...
from deeppcb.target import read_list, file_name, process_crop, process_filter, process_align_camera, process_gmm_seg, get_blank_mask, get_gmm_img, get_feature_img,process_spilt_align_camera
...
def spilt_align(ctx,img):
    ...
    img = cv2.imread('/root/project/DenseMatching/demos/warped.jpg')
    img = img[y0:y1, x0:x1]
    ...
    for id in range(spilt_part):
        ...
        for col in range(col_num):
            for row in range(row_num):
                ...
                base_name_list.append(name + f'_{col}_{row}_'+cam_id)
            ...
            distmap_list.append(tpl_distmap_crop_img)
            black_mask_list.append(black_mask_crop_img)
            base_name_list.append(name + f'_{col}_row_'+cam_id)
        ...
        for row in range(row_num):
            ...
            distmap_list.append(tpl_distmap_crop_img)
            black_mask_list.append(black_mask_crop_img)
            base_name_list.append(name + f'_{row}_col_'+cam_id)
